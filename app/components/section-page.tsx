...
import {
  ArrowRight, Check, CircleAlert, Download,
...
function Nav({lang,active}:{lang:Lang;active?:"auctions"|"selection"|"business"}){
...
function Auctions({lang}:{lang:Lang}){
  ...
  return <><Nav lang={lang} active="auctions"/><PageHead title={t.all} sub="Découvrez toutes les enchères en cours mises à jour en temps réel."/><main className="pageSurface"><div className="innerWrap auctionLayout">
    <aside className="filterRail"><h2><SlidersHorizontal/> {t.filters}</h2>
      <label>Marque<select value={brand} onChange={event=>{setBrand(event.target.value);setVisibleCount(AUCTION_PAGE_SIZE)}}>{["Tous","BMW","Mercedes","Audi","Peugeot","Renault"].map(value=><option key={value}>{value}</option>)}</select></label>
      <p className="filterNote">Les annonces affichées sont les enchères actives disponibles.</p>
    </aside>
    <section className="results"><div className="resultTools"><label><Search/><input aria-label="Rechercher une marque ou un modèle" value={query} onChange={e=>{setQuery(e.target.value);setVisibleCount(AUCTION_PAGE_SIZE)}} placeholder="Rechercher une marque, un modèle…"/></label><div className="brandChips">{["Tous","BMW","Mercedes","Audi","Peugeot","Renault"].map(value=><button className={brand===value?"active":""} aria-pressed={brand===value} onClick={()=>{setBrand(value);setVisibleCount(AUCTION_PAGE_SIZE)}} key={value}>{value}</button>)}</div></div>
      ...
      {mode!=="loading"&&shown.length===0&&!hasMore&&<Empty lang={lang} message={mode==="error"?t.unavailable:mode==="empty"?t.noAuctions:undefined}/>}
    </section>
  </div></main></>;
}
...
function Selection({lang}:{lang:Lang}){
  const t=ui[lang];
  return <><Nav lang={lang} active="selection"/><PageHead title={t.selection} sub="Retrouvez ici tous vos véhicules favoris."/><main className="pageSurface"><div className="innerWrap">
    <div className="sectionLine"><h2>{t.saved}</h2></div>
    <Empty lang={lang} message="Aucun véhicule enregistré pour le moment."/>
  </div></main></>;
}
...
function Auth({lang,register}:{lang:Lang;register:boolean}){
  ...
  return <><Nav lang={lang}/><main className="authPage"><form className="authCard" onSubmit={event=>{event.preventDefault();setStatus(register?"L’inscription n’est pas encore disponible.":"La connexion n’est pas encore disponible.")}}>
    ...
    <label>Email<input type="email" required autoComplete="email" placeholder="votre@email.com"/></label>
    <label>Mot de passe<input type="password" required minLength={6} autoComplete={register?"new-password":"current-password"} placeholder="••••••••"/></label>
    <label className="authCheck"><input type="checkbox" required={register}/>{register?"J’accepte les CGU et la Politique de confidentialité":"Se souvenir de moi"}</label>
    <button className="primaryBtn" type="submit">{register?"S’inscrire":"Se connecter"}</button>
    {status&&<p className="formStatus" role="status">{status}</p>}
    <span>{register?"Déjà un compte ?":"Pas encore de compte ?"} <Link href={href(register?"/login":"/register",lang)}>{register?"Se connecter":"S’inscrire"}</Link></span>
  </form></main></>;
}
...
function Pricing({lang}:{lang:Lang}){
  const t=ui[lang];
  const plans=[
    {n:"GRATUIT",p:"0 €",f:["Score LotRank","Résumé du marché","Suivi de 5 véhicules"]},
    {n:"PRO",p:"14,99 € / mois",f:["Coût total","Historique 90 jours","Alertes prioritaires"]},
    {n:"BUSINESS",p:"49 € / mois",f:["Rapports complets","Exports","Filtres professionnels"]},
  ];
  return <><Nav lang={lang} active="business"/><PageHead title={t.pricing} sub="Plus d’outils, plus de données, plus de succès."/><main className="pageSurface"><div className="innerWrap pricingShell">
...
function Checkout({lang}:{lang:Lang}){const t=ui[lang];return <><Nav lang={lang} active="business"/><PageHead title={t.payment} sub="Souscription LotRank"/><main className="pageSurface"><div className="innerWrap"><div className="emptyState"><LockKeyhole/><h2>La souscription en ligne n’est pas encore disponible.</h2><p>Aucun paiement ne sera demandé et aucun abonnement ne sera activé.</p><Link className="outlineBtn" href={href("/pricing",lang)}>Retour aux offres</Link></div></div></main></>;}
...
export function SectionPage({section,lang,initialQuery}:{section:string;lang:Lang;initialQuery:string}){
  let page;
  if(section==="auctions")page=<Auctions lang={lang}/>;
  else if(section==="selection")page=<Selection lang={lang}/>;
  else if(section==="compare")page=<Compare lang={lang}/>;
  else if(section==="login"||section==="register")page=<Auth lang={lang} register={section==="register"}/>;
  else if(section==="pricing")page=<Pricing lang={lang}/>;
  else if(section==="checkout")page=<Checkout lang={lang}/>;
  else if(section==="business")page=<Business lang={lang}/>;
  else if(section==="sources")page=<Sources lang={lang}/>;
  else if(section==="how")page=<How lang={lang}/>;
  else page=<SearchPage lang={lang} initialQuery={initialQuery}/>;
  return <>{page}<SiteFooter lang={lang}/></>;
}
