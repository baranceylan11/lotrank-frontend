...
const ui = {
  ...
  en: { auctions:"Auctions", selection:"Watchlist", business:"Business reports", sources:"Sources", how:"How it works", login:"Log in", all:"All auctions", search:"Search", filters:"Filters", compare:"Compare", pricing:"Choose your plan", payment:"Complete subscription", saved:"Saved vehicles", noResult:"No vehicle found" },
  zh: { auctions:"拍卖", selection:"关注列表", business:"商业报告", sources:"数据源", how:"工作原理", login:"登录", all:"全部拍卖", search:"搜索", filters:"筛选", compare:"比较", pricing:"选择方案", payment:"完成订阅", saved:"已保存车辆", noResult:"未找到车辆" },
} as const;
...
function VehicleCard({v,lang,selected,onSelect}:{v:(typeof vehicles)[number];lang:Lang;selected?:boolean;onSelect?:()=>void}){
  return <article className="listCar"><div className="listPhoto"><img src={v.image} alt={`${v.brand} ${v.model}`}/><em>LotRank {v.score}</em><span>● LIVE</span>{onSelect&&<button aria-label="Select" className={selected?"pick active":"pick"} onClick={onSelect}>{selected?<Check/>:<span/>}</button>}</div><div className="listBody"><h3>{v.brand} {v.model}</h3><p>{v.year} · {v.km} · Diesel</p><footer><span><MapPin/>{v.place}</span><b>{v.price}</b></footer><Link href={`/vehicle/${v.id}?lang=${lang}`}>Voir l’analyse <ArrowRight/></Link></div></article>
}
...
function Compare({lang}:{lang:Lang}){const t=ui[lang];const list=vehicles.slice(0,3);return <><Nav lang={lang} active="selection"/><PageHead title={t.compare} sub="Les différences importantes en un coup d’œil."/><main className="pageSurface"><div className="innerWrap compareTable"><div className="compareLabels">{["Prix actuel","Score LotRank","Risque","Avantage prix","Coût total estimé","Fin de l’enchère"].map(x=><b key={x}>{x}</b>)}</div>{list.map(v=><article key={v.id}><h2>{v.brand} {v.model}</h2><strong>{v.price}</strong><strong>{v.score}/100</strong><strong>{v.score>85?"Faible":"Moyen"}</strong><strong>+{v.gain}%</strong><strong>€20 170</strong><strong>{v.time.slice(0,5)}</strong><Link className="primaryBtn" href={`/vehicle/${v.id}?lang=${lang}`}>Voir le véhicule</Link></article>)}</div></main></>}
...
function Empty({lang}:{lang:Lang}){return <div className="emptyState"><CircleAlert/><h2>{ui[lang].noResult}</h2><p>Vérifiez l’orthographe ou supprimez un filtre.</p><Link className="primaryBtn" href={href("/auctions",lang)}>Voir toutes les enchères</Link></div>}

function SearchPage({lang,initialQuery}:{lang:Lang;initialQuery:string}){const router=useRouter();const [q,setQ]=useState(initialQuery);const results=vehicles.filter(v=>`${v.brand} ${v.model}`.toLowerCase().includes(q.trim().toLowerCase()));return <><Nav lang={lang} active="auctions"/><PageHead title="Résultats de recherche" sub="La recherche reste visible et modifiable."/><main className="pageSurface"><div className="innerWrap"><form className="searchBar" onSubmit={e=>{e.preventDefault();router.push(`/search?lang=${lang}&q=${encodeURIComponent(q)}`)}}><Search/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Marque ou modèle"/><button className="primaryBtn">Rechercher</button></form><div className="sectionLine"><h2>{results.length} résultat{results.length!==1?"s":""}</h2></div>{results.length?<div className="vehicleGrid">{results.map(v=><VehicleCard v={v} lang={lang} key={v.id}/>)}</div>:<Empty lang={lang}/>}</div></main></>}
